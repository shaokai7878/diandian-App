...
module.exports = function () {
    ...
    router.get('/', function (req, res) {
        switch (req.query.action) {
            case 'del'://删除操作
                db.query('SELECT * FROM shop_table WHERE ID="' + req.query.id + '"', function (err, queryData) {
                    if (err) {
                        console.error(err);
                        ...
                        fs.unlink(queryData[0].pic_header.replace('\/files','static'), function (err) {
                            if (err) {
                                ...
                                db.query('DELETE FROM shop_table WHERE ID="'+
                ...
                break;
            case 'mod':
                db.query('SELECT * FROM shop_table',function (err,allData) {
                    if (err) {
                        console.error(err);
                        res.status(500).send({code: 500, msg: 'database error'});
                    } else {
                        db.query('SELECT * FROM shop_table WHERE ID="'+req.query.id+'"',function (err,modData) {
                            if (err) {
                                console.error(err);
                                res.status(500).send({code: 500, msg: 'database error'});
                                ...
                                res.render('admin/shop.ejs',{usersData:allData,modData:modData});
                            }
                        });
                    }
                });
                break;
            default:
                db.query('SELECT * FROM shop_table', function (err, allUsersData) {
                    if (err) {
                        console.error(err);
                        res.status(500).send({code: 500, msg: 'database error'});
                    } else {
                        res.render('admin/shop.ejs', {usersData: allUsersData});
                    }
                });
        }
    });
    router.post('/', function (req, res) {
        ...
        if(req.body.modified){//修改
            ...
            if(req.files.length>0){
                ...
                db.query('SELECT * FROM shop_table WHERE ID="'+req.body.modified+'"',function (err,modData) {
                    if (err) {
                        console.error(err);
                        res.status(500).send({code: 500, msg: 'database error'});
                    }else if(modData.length == 0){
                            ...
                            if(err){
                                ...
                                fs.rename(req.files[0].path, req.files[0].path + ext, function (err) {
                                    if (err) {
                                        ...
                                        db.query('UPDATE shop_table SET username="'+
                                            username+'",count="' + count + '",pic_header="' +
                ...
                db.query('UPDATE shop_table SET username="'+username+'",count="' + count + '" WHERE ID="'+
            ...
            fs.rename(req.files[0].path, req.files[0].path + ext, function (err) {
                if (err) {
                    ...
                    db.query('INSERT INTO shop_table (username,count,pic_header) VALUE("' + username + '","' +
