    ...
    router.post('/', function (req, res) {
        ...
        if(req.body.modified){//修改
            ...
            if(req.files.length>0){
                                ...
                                fs.rename(req.files[0].path, req.files[0].path + ext, function (err) {
                                    if (err) {
                                        ...
                                        db.query('UPDATE user_table SET username="'+
                                            username+'",password="' + password + '",pic_header="' +
                ...
                db.query('UPDATE user_table SET username="'+username+'",password="' + password + '" WHERE ID="'+
            ...
            fs.rename(req.files[0].path, req.files[0].path + ext, function (err) {
                if (err) {
                    ...
                    db.query('INSERT INTO user_table (username,password,pic_header) VALUE("' + username + '","' +
                        password + '","' + pic_header + '")', function (err, resultData) {
                        if (err) {
                            console.error(err);
                            res.status(500).send({code: 500, msg: 'database error'});
                        } else {
